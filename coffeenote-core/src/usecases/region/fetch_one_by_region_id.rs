use crate::{
    ...
    repositories::region::{FetchOneByRegionIdError, RegionRepository},
    usecases::Usecase,
};
use std::sync::Arc;
use uuid::Uuid;
...
#[cfg(test)]
mod tests {
    use super::{Error, FetchOneByRegionId, Request, Response};
    use crate::{
        entities::region::Region, repositories::region::FetchOneByRegionIdError,
        tests::harness::in_memory_repositories::region::InMemory, usecases::Usecase,
    };
    use std::sync::Arc;
    use uuid::Uuid;
    ...
    fn it_should_happen_an_error_by_repository_issue() {
        ...
        assert_eq!(
            fetch_one_by_region_id.execute(req),
            Err(Error::RepositoryError(FetchOneByRegionIdError::Unknown))
        );
    }
}
